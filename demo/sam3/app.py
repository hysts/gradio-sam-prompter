...
processor: Sam3TrackerProcessor = Sam3TrackerProcessor.from_pretrained(MODEL_ID)
model: Sam3TrackerModel = Sam3TrackerModel.from_pretrained(MODEL_ID, torch_dtype=dtype).to(device).eval()
print("Model loaded.")  # noqa: T201


def _compute_image_embeddings(image: Image.Image) -> tuple[torch.Tensor]:
...
def _predict_mask_for_object(
    obj: dict[str, Any],
    image: Image.Image,
    image_embeddings: tuple[torch.Tensor],
) -> np.ndarray | None:
...
def segment(
    ...
    """Run SAM3 inference on the current prompts."""
    ...
    for obj in prompts:
        ...
        if mask is not None:
            ...
            # Cutout: foreground on transparent background
...
with gr.Blocks(title="SAM3 Demo") as demo:
    gr.Markdown(
        "## SAM3 Prompter Demo (Real Inference)\n"
        "Drop an image, then left-click for foreground points, "
        "right-click for background points, or drag a box."
    )

    prompter = SamPrompter(label="SAM3 Prompter")
    with gr.Row():
